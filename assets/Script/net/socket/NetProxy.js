...
let NetProxy = cc.Class({
    ...
    pushCallback: function (response) {
        switch (response.act){
            ...
            case "friendInfoSync":
                ...
                break;
            case "cleanWantHelp":
                this.pushCleanWantHelp(response);
                break;
            case "acceptWantHelp":
                this.pushAcceptWantHelp(response);
                break;
            case "stole":
    ...
    /**
     * 清除通缉令协助
     * @param {PushCleanWantHelpResponse} resp
     */
    pushCleanWantHelp: function (resp) {
        // cc.log("网络请求---push--- pushCleanWantHelp 成功");
        // if(resp.rmHelpWantId) farm.game.removeHelpWant(resp.rmHelpWantId);
        // farm.eventManager.emit(farm.game.gmConst.SP_PUSH_EVENT_CLEAN_WANT_HELP_SUCCESS);
    },

    /**
     * 邀请好友协助通缉攻打
     * @param {PushAcceptWantHelpResponse} resp
     */
    pushAcceptWantHelp: function (resp) {
        // cc.log("网络请求---push--- pushAcceptWantHelp 成功");
        // if(resp.player) farm.game.addHelpWant(resp.player);
        // if(resp.news){
        //     resp.news = farm.game.addNews(resp.news);
        // }
        // farm.eventManager.emit(farm.game.gmConst.SP_PUSH_EVENT_ACCEPT_WANT_HELP_SUCCESS);
    },
    ...
    pushWantedAbout: function (resp) {
        // cc.log("网络请求---push--- pushWantedAbout 成功");
        // if(resp.news) resp.news = farm.game.addNews(resp.news);
    },
